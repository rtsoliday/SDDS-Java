...
public class sddsEdit extends JFrame implements Printable {
	...
	public static final String attributesArrayAction = "attributesArray";
	public static final String insertArrayAction = "insertArray";
	public static final String deleteArrayAction = "deleteArray";
	public static final String insertRowAction = "insertRow";
	public static final String deleteRowAction = "deleteRow";
	public static final String insertArrayRowAction = "insertArrayRow";
	public static final String deleteArrayRowAction = "deleteArrayRow";
	...
	static DefaultListModel array_dlm = new DefaultListModel();
	static String insertRows = "1";
	static SearchDialog searchDialog = null;
	static MatchList matchList = null;

	static DefaultTableModel column_dtm, parameter_dtm, array_dtm;
	static DefaultTableColumnModel column_dtcm, parameter_dtcm, array_dtcm;
	...
	private Action[] defaultActions = {
			...
			new AttributesArrayAction(),
			new InsertArrayAction(),
			new DeleteArrayAction(),
			new InsertRowAction(),
			new DeleteRowAction(),
			new InsertArrayRowAction(),
			new DeleteArrayRowAction(),
			new GoToPageAction(),
	...
	protected JMenuBar createMenubar() {
		...
		sub_m.add(mi);
		sub_m = new JMenu("Array");
		m.add(sub_m);
		mi = new JMenuItem("Attributes...");
		SetMenuAction("attributesArray", mi);
		sub_m.add(mi);
		sub_m.addSeparator();
		mi = new JMenuItem("Insert...");
		SetMenuAction("insertArray", mi);
		sub_m.add(mi);
		mi = new JMenuItem("Delete...");
		SetMenuAction("deleteArray", mi);
		sub_m.add(mi);
		sub_m = new JMenu("Column Rows");
		m.add(sub_m);
		mi = new JMenuItem("Insert...");
		...
		sub_m.add(mi);
		sub_m = new JMenu("Array Rows");
		m.add(sub_m);
		mi = new JMenuItem("Insert...");
		SetMenuAction("insertArrayRow", mi);
		sub_m.add(mi);
		mi = new JMenuItem("Delete...");
		SetMenuAction("deleteArrayRow", mi);
		sub_m.add(mi);
	...
	public sddsEdit() {
		...
		getContentPane().add(menubar, BorderLayout.NORTH); // Put menu bar on top

		// Add first Split Pane
		...
		// puts panel2 on top of second split pane (this is the parameters section)
		...
		pageChooser.addItem(Integer.valueOf(1));
		...
		// Add panel3a to the bottom of the second split pane
		JPanel panel3a = new JPanel(new BorderLayout());
		dataPane.setBottomComponent(panel3a);

		// Add third Split Pane
		JSplitPane columnarrayPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
		columnarrayPane.setOneTouchExpandable(true);
		panel3a.add(columnarrayPane, BorderLayout.CENTER);

		// Add panel3 to the top of the third split pane
		JPanel panel3 = new JPanel(gridbag);
		columnarrayPane.setTopComponent(panel3);
		...
		// Add below
		// Add panel4 to the bottom of the third split screen
		JPanel panel4 = new JPanel(gridbag);
		columnarrayPane.setBottomComponent(panel4);

		JLabel arrayLabel = new JLabel("Arrays", JLabel.CENTER);
		...
		c.gridwidth = 1;
		c.gridheight = 1;
		gridbag.setConstraints(arrayLabel, c);
		panel4.add(arrayLabel);

		arrayTable = new JTable(new DefaultTableModel());
		arrayTable.setCellSelectionEnabled(true);
		arrayTable.setPreferredScrollableViewportSize(new Dimension(240, 150));
		arrayTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);

		arrayScrollPane = new JScrollPane(arrayTable);

		JList elementHeader;
		elementHeader = new JList(array_dlm);
		elementHeader.setBackground(arrayTable.getTableHeader().getBackground());
		elementHeader.setFixedCellWidth(50);
		elementHeader.setFixedCellHeight(arrayTable.getRowHeight());
		elementHeader.setCellRenderer(new RowHeaderRenderer("array"));
		arrayScrollPane.setRowHeaderView(elementHeader);
		...
		c.gridwidth = GridBagConstraints.REMAINDER;
		c.gridheight = GridBagConstraints.REMAINDER;
		gridbag.setConstraints(arrayScrollPane, c);

		panel4.add(arrayScrollPane);

		// Add above

		array_dtm = (DefaultTableModel) arrayTable.getModel();
		column_dtm = (DefaultTableModel) columnTable.getModel();
		parameter_dtm = (DefaultTableModel) parameterTable.getModel();
		array_dtcm = (DefaultTableColumnModel) arrayTable.getColumnModel();
	...
	public void loadFile(String fileName) {
		...
		pageChooser.addItem(Integer.valueOf(1));
		for (i = 2; i <= pages; i++)
			pageChooser.addItem(Integer.valueOf(i));
	...
	public void clearPage() {
		...
		if (arrayTable.isEditing()) {
			arrayTable.getCellEditor().stopCellEditing();
		}
		...
		arrayTable.getSelectionModel().clearSelection();
		arrayTable.getColumnModel().getSelectionModel().clearSelection();
		...
		array_dtm.setNumRows(0);
		array_dtm.setDataVector(new Object[0][0], new Object[0]);
	}
	...
	public void loadPage(int page) {
		...
		Object[][] arrayMajorOrderValues = null;
		int i, j, numberOfColumns = 0, numberOfParameters = 0, numberOfArrays;
		int rows;
		...
		if (columnNames != null) {
			...
			column_dlm.clear();
			for (i = 0; i < rows; i++) {
				column_dlm.addElement(Integer.valueOf(i + 1));
		...
		if (arrayNames != null) {
			...
			numberOfArrays = Array.getLength(arrayNames);
			elements = new int[numberOfArrays];
			for (i = 0; i < numberOfArrays; i++) {
				dimensions = sdds.getArrayDimensions(i);
				dim = sdds.getArrayDim(page, i);
				elements[i] = 0;
				for (j = 0; j < dimensions; j++) {
					elements[i] = elements[i] + dim[j];
				}
				if (elements[i] > maxElements) {
					maxElements = elements[i];
				}
			}
			arrayValues = new Object[numberOfArrays][maxElements];
			arrayMajorOrderValues = new Object[maxElements][numberOfArrays];
			for (i = 0; i < numberOfArrays; i++)
				arrayValues[i] = sdds.getArrayValues(i, page, false);

			array_dlm.clear();
			for (i = 0; i < maxElements; i++) {
				array_dlm.addElement(Integer.valueOf(i + 1));
				for (j = 0; j < numberOfArrays; j++) {
					if (i < elements[j]) {
						arrayMajorOrderValues[i][j] = arrayValues[j][i];
					}
				}
			}
			array_dtm.setDataVector(arrayMajorOrderValues, arrayNames);

			ButtonHeaderRenderer renderer = new ButtonHeaderRenderer();
			TableColumn tc;
			for (i = 0; i < numberOfArrays; i++) {
				tc = array_dtcm.getColumn(i);
				if (arrayNames[i].length() <= 7) {
					tc.setPreferredWidth(75);
					tc.setWidth(75);
				} else {
					tc.setPreferredWidth((arrayNames[i].length() + 1) * 10);
					tc.setWidth((arrayNames[i].length() + 1) * 10);
				}
				tc.setHeaderRenderer(renderer);
				if ((DefaultCellEditor) (array_dtcm.getColumn(i).getCellEditor()) == null) {
					dce = new DefaultCellEditor(new JTextField());
					dce.setClickCountToStart(1);
					array_dtcm.getColumn(i).setCellEditor((TableCellEditor) dce);
				}
			}
			JTableHeader header = arrayTable.getTableHeader();
			header.addMouseListener(new ArrayHeaderListener(header, renderer));
	...
	class ArrayHeaderListener extends MouseAdapter {
		JTableHeader header;
		ButtonHeaderRenderer renderer;

		ArrayHeaderListener(JTableHeader header, ButtonHeaderRenderer renderer) {
			this.header = header;
			this.renderer = renderer;
		}
		...
		public void mousePressed(MouseEvent e) {
			...
			clearArraySelections();
			arrayTable.setColumnSelectionInterval(col, col);
			arrayTable.getSelectionModel().addSelectionInterval(0, array_dtm.getRowCount() - 1);
		}
	...
	public boolean savePage() {
		...
		int i, j, numberOfColumns = 0, numberOfParameters, numberOfArrays;
		int rows, cols, pars;
				...
				for (j = 0; j < pars; j++) {
					if (parameterNames[i].equals((String) parameter_dlm.getElementAt(j))) {
						if (types[i] == SDDSUtil.SDDS_CHARACTER) {
							if (!sdds.setParameter(i, SDDSUtil.returnCharValueFromOctalCode(parameterValues[j]),
									currentPage + 1)) {
								appendStatus("Error: " + sdds.getErrors());
								return false;
							}
		...
		if (arrayNames != null) {
			...
			Object o;
			numberOfArrays = Array.getLength(arrayNames);
			rows = array_dtm.getRowCount();
			cols = array_dtm.getColumnCount(); /* may be different then numberOfArrays */
			elements = new int[cols];

			arrayValues = new String[cols][rows];
			for (i = 0; i < cols; i++) {
				for (j = 0; j < rows; j++) {
					o = array_dtm.getValueAt(j, i);
					if ((o != null) && (!o.toString().equals(s))) {
						arrayValues[i][j] = o.toString();
						elements[i] = j + 1;
					} else {
						break;
					}
			...
			for (i = 0; i < numberOfArrays; i++) {
				for (j = 0; j < cols; j++) {
					if (arrayNames[i].equals(array_dtm.getColumnName(j))) {
						sdds.setArrayDim(currentPage + 1, i, Arrays.copyOfRange(elements, j, j + 1));
						if (!sdds.setArray(i, Arrays.copyOfRange(arrayValues[j], 0, elements[j]), currentPage + 1)) {
							appendStatus("Error: " + sdds.getErrors());
							return false;
						}
						break;
	...
	class AttributesParameterAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (i >= 0) {
				...
				sddsEditAttributesDialog sddsEditAttributesDialog = new sddsEditAttributesDialog(frame, 1);
				sddsEditAttributesDialog.pack();
				sddsEditAttributesDialog.setAttributesMode(1);
	...
	class InsertParameterAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			sddsEditAttributesDialog sddsEditAttributesDialog = new sddsEditAttributesDialog(frame, 1);
			sddsEditAttributesDialog.pack();
			sddsEditAttributesDialog.setInsertMode(1);
			sddsEditAttributesDialog.setLocationRelativeTo(frame);
			sddsEditAttributesDialog.setVisible(true);
			if (sddsEditAttributesDialog.getName() == null)
				return;
	...
	class AttributesColumnAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (i >= 0) {
				...
				sddsEditAttributesDialog sddsEditAttributesDialog = new sddsEditAttributesDialog(frame, 2);
				sddsEditAttributesDialog.pack();

				sddsEditAttributesDialog.setAttributesMode(2);
				sddsEditAttributesDialog.setName(name);
	...
	class InsertColumnAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			sddsEditAttributesDialog sddsEditAttributesDialog = new sddsEditAttributesDialog(frame, 2);
			sddsEditAttributesDialog.pack();
			sddsEditAttributesDialog.setInsertMode(2);
			sddsEditAttributesDialog.setLocationRelativeTo(frame);
			sddsEditAttributesDialog.setVisible(true);
			if (sddsEditAttributesDialog.getName() == null)
				return;
	...
	class AttributesArrayAction extends AbstractAction {

		AttributesArrayAction() {
			super(attributesArrayAction);
		}

		public void actionPerformed(ActionEvent e) {
			int i, j, k;
			i = arrayTable.getSelectedColumn();
			if (i >= 0) {
				String name = arrayTable.getColumnName(i);
				int index = sdds.getArrayIndex(name);
				if (index < 0) {
					appendStatus("Error: " + sdds.getErrors());
					return;
				}
				sddsEditAttributesDialog sddsEditAttributesDialog = new sddsEditAttributesDialog(frame, 3);
				sddsEditAttributesDialog.pack();

				sddsEditAttributesDialog.setAttributesMode(3);
				sddsEditAttributesDialog.setName(name);
				sddsEditAttributesDialog.setSymbol(sdds.getArraySymbol(index));
				sddsEditAttributesDialog.setUnits(sdds.getArrayUnits(index));
				sddsEditAttributesDialog.setFormatString(sdds.getArrayFormatString(index));
				sddsEditAttributesDialog.setType(sdds.getArrayType(index));
				sddsEditAttributesDialog.setFieldLength(sdds.getArrayFieldLength(index));
				sddsEditAttributesDialog.setGroupName(sdds.getArrayGroupName(index));
				// sddsEditAttributesDialog.setDimensions(sdds.getArrayDimensions(index));

				sddsEditAttributesDialog.setLocationRelativeTo(frame);
				sddsEditAttributesDialog.setVisible(true);
				if (sddsEditAttributesDialog.getName() == null)
					return;
				sdds.setArraySymbol(index, sddsEditAttributesDialog.getSymbol());
				sdds.setArrayUnits(index, sddsEditAttributesDialog.getUnits());
				sdds.setArrayDescription(index, sddsEditAttributesDialog.getDescription());
				sdds.setArrayFormatString(index, sddsEditAttributesDialog.getFormatString());
				sdds.setArrayFieldLength(index, sddsEditAttributesDialog.getFieldLength());
				sdds.setArrayGroupName(index, sddsEditAttributesDialog.getGroupName());
				// sdds.setArrayDimensions(index, sddsEditAttributesDialog.getDimensions());

				sdds.renameArray(name, sddsEditAttributesDialog.getName());
				String[] arrayNames = null;
				j = array_dtm.getColumnCount();
				arrayNames = new String[j];
				TableColumn tc;
				int width;
				int[] widtharray = new int[j];
				for (k = 0; k < j; k++) {
					if (i != k)
						arrayNames[k] = array_dtm.getColumnName(k);
					else
						arrayNames[k] = sddsEditAttributesDialog.getName();
					tc = array_dtcm.getColumn(k);
					widtharray[k] = tc.getWidth();
				}
				array_dtm.setColumnIdentifiers(arrayNames);
				for (k = 0; k < j; k++) {
					tc = array_dtcm.getColumn(k);
					if (k == i) {
						width = (arrayNames[k].length() + 1) * 10;
						if (width > widtharray[k])
							widtharray[k] = width;
					}
					tc.setPreferredWidth(widtharray[k]);
	...
	class InsertArrayAction extends AbstractAction {

		InsertArrayAction() {
			super(insertArrayAction);
		}

		public void actionPerformed(ActionEvent e) {
			sddsEditAttributesDialog sddsEditAttributesDialog = new sddsEditAttributesDialog(frame, 3);
			sddsEditAttributesDialog.pack();
			sddsEditAttributesDialog.setInsertMode(3);
			sddsEditAttributesDialog.setLocationRelativeTo(frame);
			sddsEditAttributesDialog.setVisible(true);
			if (sddsEditAttributesDialog.getName() == null)
				return;
			int rows, i, pages, index;
			String[] emptyData;
			String emptyString = "";
			int[] dim;
			dim = new int[1];
			dim[0] = 0;
			rows = array_dtm.getRowCount();
			index = sdds.defineArray(sddsEditAttributesDialog.getName(), sddsEditAttributesDialog.getSymbol(),
					sddsEditAttributesDialog.getUnits(), sddsEditAttributesDialog.getDescription(),
					sddsEditAttributesDialog.getFormatString(), sddsEditAttributesDialog.getGroupName(),
					sddsEditAttributesDialog.getSDDSType(), sddsEditAttributesDialog.getFieldLength(),
					sddsEditAttributesDialog.getDimensions());
			if (index == -1) {
				appendStatus("Error: " + sdds.getErrors());
				return;
			}
			...
			for (i = 0; i < rows; i++)
				emptyData[i] = emptyString;
			array_dtm.addColumn(sddsEditAttributesDialog.getName(), emptyData);
			ButtonHeaderRenderer renderer = new ButtonHeaderRenderer();
			TableColumn tc;
			int numberOfArrays = array_dtcm.getColumnCount();
			String[] arrayNames = sdds.getArrayNames();
			DefaultCellEditor dce;
			for (i = 0; i < numberOfArrays; i++) {
				tc = array_dtcm.getColumn(i);
				if (arrayNames[i].length() <= 7) {
			...
			pages = sdds.pageCount();
			for (i = 1; i <= pages; i++) {
				sdds.makeBlankArrayPage(i, index);
				sdds.setArrayDim(i, index, dim);
			}
			/*
			 * pages = sdds.pageCount();
			 * for (i=1;i<=pages;i++) {
			 ...
			 * for (j=0;j<rows;j++)
			 * emptyData[j] = emptyString;
			 * if (!sdds.setArray(sddsEditAttributesDialog.getName(),emptyData,i)) {
			 * appendStatus("Error: " + sdds.getErrors());
			 * return;
			 * }
			 * }
			 * }
			 */
			appendStatus("array added");
		}
	}

	class DeleteArrayAction extends AbstractAction {

		DeleteArrayAction() {
			super(deleteArrayAction);
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			i = arrayTable.getSelectedColumn();
			while (i >= 0) {
				tc = arrayTable.getColumn(arrayTable.getColumnName(i));
				sdds.deleteArray(arrayTable.getColumnName(i));
				arrayTable.removeColumn(tc);
				appendStatus("array removed");
				i = arrayTable.getSelectedColumn();
			}
		}
	}
			...
			if ((selectedRowCount > 0) || (columnTable.getRowCount() == 0)) {
				...
				if (rows > 0) {
					...
					for (i = 0; i < rows; i++) {
						...
						column_dlm.addElement(Integer.valueOf(column_dtm.getRowCount()));
					}
					if (rows == 1)
	...
	class InsertArrayRowAction extends AbstractAction {

		InsertArrayRowAction() {
			super(insertArrayRowAction);
		}

		public void actionPerformed(ActionEvent e) {

			int selectedRowCount;
			selectedRowCount = arrayTable.getSelectedRowCount();
			if ((selectedRowCount > 0) || (arrayTable.getRowCount() == 0)) {
				TextInsertDialog textInsertDialog = new TextInsertDialog(frame);
				textInsertDialog.pack();
				textInsertDialog.setTitleInfo("Insert Rows");
				textInsertDialog.setEntryLabel("Number of new rows:");
				...
				if (rows > 0) {
					...
					cols = array_dtm.getColumnCount();
					emptyData = new String[cols];
					for (i = 0; i < cols; i++)
						emptyData[i] = emptyString;
					selectedIndex = arrayTable.getSelectedRow();
					if (selectedIndex == -1)
						selectedIndex = 0;
					if (arrayTable.isEditing()) {
						arrayTable.getCellEditor().stopCellEditing();
					...
					for (i = 0; i < rows; i++) {
						array_dtm.insertRow(selectedIndex, emptyData);
						array_dlm.addElement(Integer.valueOf(array_dtm.getRowCount()));
					}
					if (rows == 1)
						appendStatus("Row added");
					else
	...
	class DeleteArrayRowAction extends AbstractAction {

		DeleteArrayRowAction() {
			super(deleteArrayRowAction);
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (selectedRowCount > 0) {
				...
				for (i = 0; i < cols; i++)
					emptyData[i] = emptyString;
				selectedIndexes = new int[selectedRowCount];
				selectedIndexes = arrayTable.getSelectedRows();
				if (arrayTable.isEditing()) {
					arrayTable.getCellEditor().stopCellEditing();
				}
				...
				for (i = selectedRowCount - 1; i >= 0; i--) {
					// sdds.deleteRow(currentPage+1, selectedIndexes[i]+1);
					array_dtm.removeRow(selectedIndexes[i]);
				}
	...
	class InsertPageAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			columnNames = sdds.getColumnNames();
			arrayNames = sdds.getArrayNames();
			if ((parameterNames == null) || (columnNames == null) || (arrayNames == null)) {
				appendStatus("Unable to insert page");
			} else if ((Array.getLength(parameterNames) == 0) && (Array.getLength(columnNames) == 0)
					&& (Array.getLength(arrayNames) == 0)) {
				appendStatus("Define parameters, columns or arrays first");
			} else {
				if (savePage()) {
					clearPage();
					...
					pageChooser.addItem(Integer.valueOf(sdds.pageCount()));
	...
	public static void clearArraySelections() {
		arrayTable.getSelectionModel().clearSelection();
	}
	...
	class RowHeaderRenderer extends JButton implements ListCellRenderer {
		...
		RowHeaderRenderer(String tabletype) {
			...
			} else if (tabletype.equals("column")) {
				header = columnTable.getTableHeader();
			} else {
				arrTable = true;
				header = arrayTable.getTableHeader();
			}
			setOpaque(true);
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (isSelected && cellHasFocus) {
				...
				if (paraTable) {
					...
					arrayTable.getSelectionModel().clearSelection();
					arrayTable.getSelectionModel().setSelectionInterval(index, index);
					arrayTable.setColumnSelectionInterval(0, 0);
					// arrayTable.setColumnSelectionInterval(0,
					// ((DefaultTableModel)arrayTable.getModel()).getArrayCount() - 1);
				} else {
					if (columnTable.isEditing()) {
						columnTable.getCellEditor().stopCellEditing();
					}
					...
					columnTable.setColumnSelectionInterval(0,
							((DefaultTableModel) columnTable.getModel()).getColumnCount() - 1);
				}
			}
			getModel().setPressed(isPressed);
			getModel().setArmed(isPressed);
	...
	public int printold(Graphics g, PageFormat pageFormat,
		...
		double headerHeightOnPage = columnTable.getTableHeader().getHeight() * scale;
		...
		int totalNumPages = (int) Math.ceil(((double) columnTable.getRowCount()) / numRowsOnAPage);
