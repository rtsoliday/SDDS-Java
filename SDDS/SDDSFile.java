...
public class SDDSFile {
     ...
     * Create a new SDDS file that can be used to read and write SDDS files.
     * 
     * @param binary <code>true</code> if binary output, <code>false</code> if ascii
     *               output
     */
     ...
     * 
     * @param filename file name to read or write data
     * @param binary   <code>true</code> if binary output, <code>false</code> if
     *                 ascii output
     */
     ...
     *                      <code>false</code> if big endian input and output. Not
     *                      used with ascii files.
     */
    ...
    /**
     * Set for fixed number of rows in column data or dynamic. This method will have
     * no effect
    ...
    /**
     * Switch writing of column data from one file to a new file without having to
     * redefine parameters,
     * arrays and columns. This is useful for when noRowCount is set and the number
     * of rows has grown
     * rather large and continuing the data in a new file would seem appropriate.
     * The parameter and array data
     * is duplicated in the new file and the column data headers are duplicated in
     * the new file
     * without carrying the actual column data from the old file over. Fresh row
     * data can be added one
    ...
    public boolean switchToNewFile(String fileName) {
        /*
         * delete the row data but keep the row definitions and all arrays and
         * parameters
         */
        noRowCount = true;
        int rowCount;
            ...
            for (int page = 1; page <= pcount; page++) {
                ...
                switch (type) {
                    ...
                    case SDDS_ULONG64:
                         ...
                         * huge numbers
                         */
                        v = ((Long) object).longValue();
                        if (v < 0)
                            v = 0;
                        if (!setParameter(i, v, page))
                            return false;
                        break;
                    case SDDS_LONG:
                        if (!setParameter(i, ((Long) object).longValue(), page))
    ...
    public boolean parseHeaders() {
        ...
        try {
            ...
            if ((layoutVersion != 1) && (layoutVersion != 2) && (layoutVersion != 3) && (layoutVersion != 4)
                    && (layoutVersion != 5)) {
                ...
                switch (header) {
                    case 1:
                        if (token.equals("&end")) {
                            ...
                            if (defineParameter(name, symbol, units, description, format_string, type,
                                    fixed_value) == -1) {
                    ...
                    case 2:
                        if (token.equals("&end")) {
                            ...
                            if (defineColumn(name, symbol, units, description, format_string, type,
                                    field_length) == -1) {
            ...
            try {
                while (true) {
                    ...
                    if (rows == Integer.MAX_VALUE) {
                        setError("Row count exceeds limit in Java SDDS (readBinaryData)");
                        return false;
                    }
                    for (i = 0; i < parameterCount; i++) {
                        ...
                        switch (parDataTypes[i]) {
                            ...
                            case SDDS_ULONG:
                                ...
                                break;
                            case SDDS_LONG64:
                                setParameter(i, dis.readLong(), page);
                                break;
                            case SDDS_ULONG64:
                                v = dis.readLong();
                                if (v < 0)
                                    v = 0;
                                setParameter(i, v, page);
                                break;
                            case SDDS_FLOAT:
                    ...
                    for (i = 0; i < arrayCount; i++) {
                        ...
                        switch (arrDataTypes[i]) {
                            ...
                            case SDDS_LONG64:
                            case SDDS_ULONG64:
                                arrLongValues[i] = new long[arraySize];
                                break;
                            case SDDS_FLOAT:
                            case SDDS_DOUBLE:
                            case SDDS_LONGDOUBLE:
                        ...
                        for (j = 0; j < arraySize; j++) {
                            switch (arrDataTypes[i]) {
                                case SDDS_SHORT:
                                    arrLongValues[i][j] = (long) dis.readShort();
                                    break;
                                case SDDS_USHORT:
                                    arrLongValues[i][j] = (long) ((int) (dis.readShort() & 0xffff));
                                    break;
                                case SDDS_LONG:
                                    arrLongValues[i][j] = (long) dis.readInt();
                                    break;
                                case SDDS_ULONG:
                                    arrLongValues[i][j] = (long) ((long) (dis.readInt() & 0xffffffffL));
                                    break;
                                case SDDS_LONG64:
                                    arrLongValues[i][j] = (long) dis.readLong();
                                    break;
                                case SDDS_ULONG64:
                                    arrLongValues[i][j] = (long) dis.readLong();
                                    if (arrLongValues[i][j] < 0)
                                        arrLongValues[i][j] = 0;
                                    break;
                                case SDDS_FLOAT:
                        ...
                        switch (arrDataTypes[i]) {
                            ...
                            case SDDS_LONG64:
                            case SDDS_ULONG64:
                                if (!setArray(i, arrLongValues[i], page)) {
                            ...
                            switch (colDataTypes[i]) {
                                case SDDS_SHORT:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) dis.readShort();
                                    break;
                                case SDDS_USHORT:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ((int) (dis.readShort() & 0xffff));
                                    break;
                                case SDDS_LONG:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) dis.readInt();
                                    break;
                                case SDDS_ULONG:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ((long) (dis.readInt() & 0xffffffffL));
                                    break;
                                case SDDS_LONG64:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) dis.readLong();
                                    break;
                                case SDDS_ULONG64:
                                    for (row = 0; row < rows; row++) {
                                        longValues[i][row] = (long) dis.readLong();
                                        if (longValues[i][row] < 0)
                                            longValues[i][row] = 0;
                                    }
                                    break;
                                case SDDS_FLOAT:
                    ...
                    if (rows > 0) {
                        for (i = 0; i < columnCount; i++) {
                            switch (colDataTypes[i]) {
                                ...
                                case SDDS_LONG64:
                                case SDDS_ULONG64:
                                    if (!setColumn(i, longValues[i], page)) {
                ...
                while (true) {
                    ...
                    for (i = 0; i < parameterCount; i++) {
                        ...
                        switch (parDataTypes[i]) {
                            ...
                            case SDDS_ULONG:
                                ...
                                break;
                            case SDDS_LONG64:
                                setParameter(i, ledis.readLong(), page);
                                break;
                            case SDDS_ULONG64:
                                v = ledis.readLong();
                                if (v < 0)
                    ...
                    for (i = 0; i < arrayCount; i++) {
                        ...
                        for (j = 0; j < arraySize; j++) {
                            switch (arrDataTypes[i]) {
                                case SDDS_SHORT:
                                    arrLongValues[i][j] = (long) ledis.readShort();
                                    break;
                                case SDDS_USHORT:
                                    arrLongValues[i][j] = (long) ((int) (ledis.readShort() & 0xffff));
                                    break;
                                case SDDS_LONG:
                                    arrLongValues[i][j] = (long) ledis.readInt();
                                    break;
                                case SDDS_ULONG:
                                    arrLongValues[i][j] = (long) ((long) (ledis.readInt() & 0xffffffffL));
                                    break;
                                case SDDS_LONG64:
                                    arrLongValues[i][j] = (long) ledis.readLong();
                                    break;
                                case SDDS_ULONG64:
                                    arrLongValues[i][j] = (long) ledis.readLong();
                                    if (arrLongValues[i][j] < 0)
                    ...
                    if (columnMajorOrder) {
                        for (i = 0; i < columnCount; i++) {
                            switch (colDataTypes[i]) {
                                case SDDS_SHORT:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ledis.readShort();
                                    break;
                                case SDDS_USHORT:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ((int) (ledis.readShort() & 0xffff));
                                    break;
                                case SDDS_LONG:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ledis.readInt();
                                    break;
                                case SDDS_ULONG:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ((long) (ledis.readInt() & 0xffffffffL));
                                    break;
                                case SDDS_LONG64:
                                    for (row = 0; row < rows; row++)
                                        longValues[i][row] = (long) ledis.readLong();
                                    break;
                                case SDDS_ULONG64:
                                    for (row = 0; row < rows; row++) {
                                        longValues[i][row] = (long) ledis.readLong();
    ...
    public boolean readAsciiData() {
        ...
        longArrValues = new long[arrayCount];
                ...
                if (par < parameterCount) {
                    ...
                    if (arraySize > 0) {
                        switch (arrDataTypes[arr]) {
                            ...
                            case SDDS_LONG64:
                            case SDDS_ULONG64:
                                longArrValues = new long[arraySize];
                                break;
                            case SDDS_FLOAT:
                            case SDDS_DOUBLE:
                            case SDDS_LONGDOUBLE:
                    ...
                    for (i = 0; i < arraySize; i++) {
                        ...
                        switch (arrDataTypes[arr]) {
                            ...
                            case SDDS_LONG64:
                            case SDDS_ULONG64:
                                longArrValues[i] = (Long.valueOf(st.sval)).longValue();
                                break;
                            case SDDS_FLOAT:
                            case SDDS_DOUBLE:
                    ...
                    switch (arrDataTypes[arr]) {
                        ...
                        case SDDS_LONG64:
                        case SDDS_ULONG64:
                            if (!setArray(arr, longArrValues, page)) {
                                setError("unable to set array (readAsciiData)");
                                return false;
                            }
                            break;
                        ...
                        while (col < columnCount) {
                            ...
                            switch (datatypes[col]) {
                                ...
                                case SDDS_CHARACTER:
                                    if (noRowCount && row >= Array.getLength(stringValues[col]))
                                        stringValues[col] = (String[]) SDDSUtil.resize(stringValues[col], row + 1);
                                    stringValues[col][row] = String
                                            .valueOf(SDDSUtil.returnCharValueFromOctalCode(st.sval));
                                    break;
                                case SDDS_SHORT:
                                case SDDS_USHORT:
                                case SDDS_LONG:
                                ...
                                case SDDS_LONG64:
                                case SDDS_ULONG64:
                                    if (noRowCount && row >= Array.getLength(longValues[col]))
                                        longValues[col] = (long[]) SDDSUtil.resize(longValues[col], row + 1);
                                    longValues[col][row] = (Long.valueOf(st.sval)).longValue();
                                    break;
                                case SDDS_FLOAT:
                                case SDDS_DOUBLE:
    ...
    public void makeBlankArrayPage(int page, int index) {
        arrayObject[index].makeBlankPages(page);
    }
     ...
     * @param page          the page of the column, must be greater then
     *                      <code>0</code>
     ...
     */
    public int defineParameter(String name, String symbol, String units, String description, String format_string,
        ...
        parameterObject[parameterCount].setupParameter(name, symbol, units, description, format_string, type,
                fixed_value);
        parameterCount++;
    ...
    public int defineArray(String name, String symbol, String units, String description,
        ...
        arrayObject[arrayCount].setupArray(name, symbol, units, description, format_string, group_name, type,
                field_length, dimensions);
        arrayCount++;
        return arrayCount - 1;
    }
    ...
    public boolean defineArrays(String[] names, String[] types, int[] dimensions) {
        ...
        for (i = 0; i < n; i++) {
            if (defineArray(names[i], null, null, null, null, null, SDDSUtil.identifyType(types[i]), 0,
                    dimensions[i]) == -1) {
                return false;
            }
        }
     ...
     *         <code>-1</code> if an error occurs
     */
    public int defineColumn(String name, String symbol, String units, String description, String format_string,
            String type, int field_length) {
    ...
    public boolean setArray(int arrayIndex, Object[] values, int page) {
        ...
        switch (type) {
            ...
            case SDDS_LONG64:
            case SDDS_ULONG64:
                arrayObject[arrayIndex].setValues(SDDSUtil.castArrayAsLong(values, type), page);
                return true;
            case SDDS_FLOAT:
            case SDDS_DOUBLE:
    ...
    /**
     * Set the values of the column at the specified page starting at the specified
     * row.
     * 
     * @param name     name of an existing column
     * @param values   array of values to be set
    ...
    public boolean setColumn(int columnIndex, Object[] values, int page, int startRow) {
        ...
        switch (type) {
            ...
            case SDDS_LONG64:
            case SDDS_ULONG64:
                return setColumn(columnIndex, SDDSUtil.castArrayAsLong(values, type), page, startRow);
            case SDDS_FLOAT:
            case SDDS_DOUBLE:
        ...
        if (asciiFile) {
            ...
            for (i = 0; i < parameterCount; i++) {
                if (!parameterObject[i].writeValue(dos, page, true)) {
                    setError("invalid page number (" + page + ") for parameter " + parameterObject[i].getName()
                            + " (writePage)");
                    try {
                        dos.flush();
                    } catch (IOException e) {
            ...
            if (!noRowCount) {
                /*
                 * We are using 32bit integer row count instead of 64bit because Java arrays are
                 * limited in size
                 */
                if (columnCount > 0) {
                    try {
                        ...
                        for (j = 1; j <= endRow; j++) {
                            for (i = 0; i < columnCount; i++) {
                                if (!columnObject[i].writeElement(dos, page, j, true)) {
                                    setError("invalid column elemant at page (" + page + ") row (" + j + ") for column "
                                            + columnObject[i].getName() + " (writePage)");
                                    try {
                                        dos.flush();
                                    } catch (IOException e) {
                                    }
        ...
        if (size < 0) {
            ...
            for (int i = 0; i < size - 1; i++) {
                try {
                    ...
                    setError("Attempt to set double value (" + v + ") for column " + column
                            + " failed. (addColumnElement)");
                    return false;
                }
            }
        }
        ...
        if (size < 0) {
            ...
            for (int i = 0; i < size - 1; i++) {
                try {
                    ...
                    setError("Attempt to set float value (" + v + ") for column " + column
                            + " failed. (addColumnElement)");
                    return false;
                }
            }
        ...
        if (size < 0) {
            ...
            for (int i = 0; i < size - 1; i++) {
                try {
                    ...
                    setError("Attempt to set long value (" + v + ") for column " + column
                            + " failed. (addColumnElement)");
                    return false;
                }
            }
        ...
        if (size < 0) {
            ...
            for (int i = 0; i < size - 1; i++) {
                try {
                    ...
                    setError("Attempt to set short value (" + v + ") for column " + column
                            + " failed. (addColumnElement)");
                    return false;
                }
            }
    ...
    /**
     * Write a new row of column data. Will write the latest data added to each
     * column.
     *
     * @return <code>true</code> if successful<br>
     *         <code>false</code> if an error occurs
    ...
    public boolean writeRow() {
        ...
        for (i = 1; i < columnCount; i++) {
            ...
            if (row != j) {
                setError("row counts for columns are not consistent (column " + i + " has " + j + " rows, column 1 has "
                        + row + " rows)");
                return false;
            }
        }
            ...
            if (columnCount > 0) {
                for (i = 0; i < columnCount; i++) {
                    if (!columnObject[i].writeElement(dos, page, row, true)) {
                        setError("invalid column elemant at page (" + page + ") row (" + row + ") for column "
                                + columnObject[i].getName() + " (writeRow)");
                        try {
                            dos.flush();
                        } catch (IOException e) {
                            return false;
