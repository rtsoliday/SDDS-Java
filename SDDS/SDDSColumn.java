...
public class SDDSColumn {
	...
	/**
	 * Create a new SDDS column that can be used to store column information for
	 * SDDS files.
	 */
	public SDDSColumn() {
		pageCount = 0;
	 ...
	 */
	public void setupColumn(String name, String symbol, String units, String description, String format_string,
			int type, int field_length) {
		columnName = name;
		columnSymbol = symbol;
		columnUnits = units;
	 ...
	 *
	 * @return The <code>int</code> value of the number of pages the column
	 *         contains.
	 */
	...
	public void setValues(double v[], int page, int startRow) {
		...
		switch (columnType) {
			...
			case SDDS_ULONG64:
				...
				for (i = 0; i < arrayLength; i++) {
					longValue[page][i + startRow] = (long) v[i];
					if (longValue[page][i + startRow] < 0)
						longValue[page][i + startRow] = 0;
				}
				break;
			case SDDS_FLOAT:
			case SDDS_DOUBLE:
			...
			if (ascii) {
				switch (columnType) {
					...
					case SDDS_ULONG:
						...
						break;
					case SDDS_LONG64:
						dos.writeBytes((long) longValue[page][row] + " ");
						break;
					case SDDS_ULONG64:
						if (longValue[page][row] < 0)
							longValue[page][row] = 0;
						dos.writeBytes((long) longValue[page][row] + " ");
						break;
					case SDDS_FLOAT:
				...
				switch (columnType) {
					...
					case SDDS_ULONG:
						...
						break;
					case SDDS_LONG64:
						dos.writeLong((long) longValue[page][row]);
						break;
					case SDDS_ULONG64:
						if (longValue[page][row] < 0)
							longValue[page][row] = 0;
		...
		try {
			switch (columnType) {
				...
				case SDDS_ULONG:
					...
					break;
				case SDDS_LONG64:
					ledos.writeLong((long) longValue[page][row]);
					break;
				case SDDS_ULONG64:
					if (longValue[page][row] < 0)
						longValue[page][row] = 0;
